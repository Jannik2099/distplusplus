...
public:
	...
	grpc::Status Distribute(grpc::ServerContext * /*context*/, const distplusplus::CompileRequest *request,
		...
		distplusplus::common::Tempfile outputFile(std::string(std::filesystem::path(request->inputfile().name()).stem()) +
												  std::string(".o"));

		std::vector<std::string> argsVec(request->argument().begin(), request->argument().end());
