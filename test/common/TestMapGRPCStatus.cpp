...
#define checkStatusMap(statusCode)                                                                                                         \
	if (mapGRPCStatus(StatusCode::statusCode) != XSTRINGIFY(statusCode)) {                                                                 \
		ret = 1;                                                                                                                           \
		std::cerr << "returned status code " << mapGRPCStatus(StatusCode::statusCode) << " didn't match " << XSTRINGIFY(statusCode)        \
				  << std::endl;                                                                                                            \
	}
...
int main() {
	...
	bool caught = false;
	try {
		mapGRPCStatus(static_cast<StatusCode>(99999)); // NOLINT(cppcoreguidelines-avoid-magic-numbers, readability-magic-numbers)
